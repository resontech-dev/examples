...
1. Install the SDK:
   ...
   ```
2. Mint a platform api key (dashboard → /profile → Developer) and provision
   an S3 bucket (Profile → Storage), then:
   ```bash
   cp .env.example .env   # fill in RESON_API_KEY, RESON_S3_KEY, RESON_S3_SECRET
   ```

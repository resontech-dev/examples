"""
Deploy the Whisper large-v3 transcription endpoint to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~3 GB checkpoint from HF Hub on first boot.
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Whisper large-v3 ASR"),
        inference=inference,

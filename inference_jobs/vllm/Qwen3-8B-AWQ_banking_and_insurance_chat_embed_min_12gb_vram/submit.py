"""
Deploy the BFSI agent bundle (Stack B: chat + embed) to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — the two
engines pull ~5.5 GB + ~1.2 GB from HF Hub on first boot.

⚠️ One-GPU pilot: Stack B and Stack A (the vision job) do NOT fit a
16 GB card together — stop one before starting the other. See
sectors/banking_and_insurance/bfsi_pilot/README.md, "Stack switching".
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "BFSI Chat+Embed (Stack B)"),
        inference=inference,
    ...
    print()
    print("[submit] For the pilot's .env (bfsi_pilot) — embed shares this deployment:")
    print(f"    CHAT_BASE_URL = {job.endpoint}")
    print(f"    CHAT_API_KEY  = {job.predict_api_key}")
    print()

    print("[submit] first boot pulls ~6.7 GB from HF Hub")

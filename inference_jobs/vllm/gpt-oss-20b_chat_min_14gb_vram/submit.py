"""
Deploy the gpt-oss-20b business chat to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~13 GB MXFP4 checkpoint from HF Hub on first boot.
...
def main() -> None:
    ...
    inference = InferenceConfig(
        ...
        # Ordered GPU preference (>= 14 GB VRAM per card); first entry is filled first.
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "gpt-oss-20b Chat"),
        inference=inference,

"""
Deploy the guarded RAG chat bundle (Qwen3-8B + Qwen3Guard + embed + rerank) to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — the four engines pull ~5.5 GB (chat AWQ) + ~16 GB (guard, fp8-quantized at load) + 2× ~1.2 GB from HF Hub on first boot.
...
def main() -> None:
    ...
    inference = InferenceConfig(
        ...
        # Ordered GPU preference (>= 22 GB VRAM per card); first entry is filled first.
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Qwen3-8B Guarded RAG Chat"),
        inference=inference,

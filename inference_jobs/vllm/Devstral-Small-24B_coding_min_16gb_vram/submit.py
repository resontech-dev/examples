"""
Deploy the Devstral Small 24B agentic coding endpoint to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~13 GB AWQ checkpoint from HF Hub on first boot.
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Devstral-Small-24B"),
        inference=inference,

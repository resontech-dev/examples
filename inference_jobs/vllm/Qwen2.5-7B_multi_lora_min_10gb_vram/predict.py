"""
Call the deployed the Qwen2.5-7B multi-LoRA endpoint endpoint — base model and LoRA adapters.
...
select an adapter per request via the ``model`` field:
    ...
    model="qwen-7b:<adapter_dir>"  → base + that adapter
...
DEFAULT_PROMPT = "Draft a friendly support reply: the customer's invoice was charged twice; we refunded the duplicate."
...
def main() -> None:
    ...
    base = os.getenv("MODEL_ID", "qwen-7b")

"""
Deploy the Qwen2.5-7B multi-LoRA endpoint to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~5.5 GB AWQ base from HF Hub; LoRA adapters resolve at request time from lora_config.dynamic_lora_loading_path.
...
def main() -> None:
    ...
    inference = InferenceConfig(
        ...
        # Ordered GPU preference (>= 10 GB VRAM per card); first entry is filled first.
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Qwen2.5-7B Multi-LoRA"),
        inference=inference,

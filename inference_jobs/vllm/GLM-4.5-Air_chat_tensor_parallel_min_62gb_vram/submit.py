"""
Deploy the GLM-4.5-Air premium chat (TP=4) to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~60 GB AWQ checkpoint from HF Hub on first boot (one download; TP shares one host).
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "GLM-4.5-Air Chat TP4"),
        inference=inference,

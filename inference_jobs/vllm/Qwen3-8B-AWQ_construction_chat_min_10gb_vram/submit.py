"""
Deploy the Qwen3-8B chat / agent-baseline job to ResonTech.
...
the ~5.5 GB AWQ checkpoint from HF Hub on first boot. The BIM assets
(sample_data/, tools/) stay local — they belong to the client side.
...
def main() -> None:
    ...
    # BYO mode: the YAML owns the scheduling declaration; serve_module.py
    # owns runtime behavior. InferenceConfig only carries submit-time knobs.
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Qwen3-8B Chat"),
        inference=inference,
    ...
    print("[submit] first boot pulls the ~5.5 GB checkpoint from HF Hub")

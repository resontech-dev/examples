"""
Deploy the Qwen2.5-VL-32B quality document-vision endpoint (TP=2) to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~19 GB AWQ checkpoint from HF Hub on first boot.
...
def main() -> None:
    ...
    inference = InferenceConfig(
        ...
        # Ordered GPU preference (>= 36 GB VRAM total, sharded across cards); first entry is filled first.
        gpu_models=["a100_40gb", "l40s", "l40"],
        max_price_per_gpu_hour=float(os.getenv("MAX_PRICE_PER_GPU_HOUR", "5.00")),
        only_mine_workers=True,
    )

    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Qwen2.5-VL-32B Vision TP2"),
        inference=inference,

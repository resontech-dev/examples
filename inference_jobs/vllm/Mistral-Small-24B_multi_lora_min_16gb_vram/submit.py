"""
Deploy the Mistral Small 24B multi-LoRA endpoint to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — vLLM pulls the ~13 GB AWQ base from HF Hub; LoRA adapters resolve at request time from lora_config.dynamic_lora_loading_path.
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Mistral-Small-24B Multi-LoRA"),
        inference=inference,

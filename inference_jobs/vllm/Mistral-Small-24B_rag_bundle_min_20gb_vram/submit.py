"""
Deploy the Mistral Small 24B RAG bundle (chat+embed+rerank) to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — the three engines pull ~13 GB + 2× ~1.2 GB from HF Hub on first boot.
...
def main() -> None:
    ...
    inference = InferenceConfig(
        ...
        # Ordered GPU preference (>= 20 GB VRAM per card); first entry is filled first.
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "Mistral-Small-24B RAG Bundle"),
        inference=inference,

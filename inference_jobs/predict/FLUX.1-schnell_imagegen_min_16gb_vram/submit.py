"""
Deploy the FLUX.1-schnell image-generation predictor to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — diffusers pulls the ~24 GB (bf16) pipeline from HF Hub in __init__.
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "FLUX.1-schnell"),
        inference=inference,
    ...
    print("[submit] first boot pulls ~24 GB of pipeline weights from HF Hub — expect many minutes")

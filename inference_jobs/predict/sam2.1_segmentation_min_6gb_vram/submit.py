"""
Deploy the SAM 2.1 promptable-segmentation predictor to ResonTech.

Ships this folder's ``inference.yaml`` (one file) and ``scripts/``
(recursively) via explicit keyword paths. No weights upload — the predictor pulls facebook/sam2.1-hiera-large (~3.5 GB) from HF Hub in __init__.
...
def main() -> None:
    ...
    job = sdk.rt_submit_inference(
        name=os.getenv("JOB_NAME", "SAM 2.1 Segmentation"),
        inference=inference,
    ...
    job.wait_until_ready(timeout=2400, poll=15)
